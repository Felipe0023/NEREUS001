...
def BLOQUE002(): # Reescalado
    st.markdown("<h4 style='text-align: center;'>Reescalado de Variables Numéricas</h4>", unsafe_allow_html=True)

    st.info("Se aplica una estandarización (Z-score) a las variables físicas para normalizar su media a 0 y su desviación estándar a 1 antes de los análisis basados en distancias (DBSCAN / ML).")

    # 1. VERIFICAR DATOS DE LA SESIÓN (Entrada: K002_datos_limpio)
    if 'K002_datos_limpio' not in st.session_state or st.session_state['K002_datos_limpio'] is None:
        ...
        st.info("Por favor, completa primero el procesamiento en la pestaña de **Limpieza de Data**.")
        return

    # Hacemos una copia profunda del dataframe limpio de entrada
    df = st.session_state['K002_datos_limpio'].copy()
    numerical_cols = ['Profundidad', 'Longitud', 'Latitud', 'Altitud', 'Cota']

    # 2. PROCESAMIENTO: ESCALADO
    scaler_ml = StandardScaler()
    
    # Aplicamos el escalado solo a las columnas numéricas existentes
    try:
        # Validar que todas las columnas requeridas estén en el dataframe antes de transformar
        missing_cols = [col for col in numerical_cols if col not in df.columns]
        if missing_cols:
            raise KeyError(f"{missing_cols}")

        df[numerical_cols] = scaler_ml.fit_transform(df[numerical_cols])
        
        # Guardamos en Session State utilizando la nueva nomenclatura correlativa (Salidas K003)
        st.session_state['K003_reescalado'] = df
        st.session_state['K003_scaler_ml'] = scaler_ml

        st.success("✅ Variables reescaladas correctamente. Guardado internamente como K003_reescalado.")

    except KeyError as e:
        st.error(f"❌ Error: No se encontraron las columnas numéricas {e} en el dataset limpio.")
        return

    # 3. INTERFAZ DE USUARIO: ESTADÍSTICAS
    with st.container(border=True):
        st.write("**Estadísticas post-escalado (Media ≈ 0, Desv. Est. ≈ 1):**")
        st.dataframe(df[numerical_cols].describe().loc[['mean', 'std']], use_container_width=True)

        # 4. PREPARAR DESCARGAS PARA USO FUTURO
        col1, col2 = st.columns(2)

        # Descarga CSV homologado a K003_reescalado.csv
        csv_esc = df.to_csv(index=False).encode('utf-8')
        col1.download_button(
            label="📥 Descargar K003_reescalado.csv",
            data=csv_esc,
            file_name="K003_reescalado.csv",
            mime="text/csv",
            use_container_width=True
        )

        # Descarga del Scaler (Joblib) homologado a K003_scaler_ml.joblib
        scaler_buffer = io.BytesIO()
        joblib.dump(scaler_ml, scaler_buffer)
        scaler_buffer.seek(0)

        col2.download_button(
            label="💾 Descargar K003_scaler_ml.joblib",
            data=scaler_buffer,
            file_name="K003_scaler_ml.joblib",
            mime="application/octet-stream",
            use_container_width=True,
            help="Descarga el modelo de escalado entrenado para usarlo en el pipeline de pronósticos con nueva data."
        )
...
def BLOQUE003():
    ...
    # 1. VERIFICAMOS QUE LA DATA REESCALADA DE ENTRADA EXISTA EN SESIÓN (Entrada: K003_reescalado)
    if 'K003_reescalado' not in st.session_state or st.session_state['K003_reescalado'] is None:
        st.warning("⚠️ No se encontraron los datos reescalados en 'K003_reescalado'.")
        st.info("Por favor, completa primero el procesamiento en la pestaña de **Reescalado de Variables (K003)**.")
        return

    # Recuperamos los datos de entrada previamente estandarizados
    df_raw = st.session_state['K003_reescalado']

    with st.container(border=True):
        # --- 1. PARAMETRIZACIÓN (Sliders en la barra lateral) ---
        st.sidebar.markdown("---")
        st.sidebar.subheader("Ajustes de DBSCAN")
        ...
        if all(col in df.columns for col in features_cols):
            # Nota: Al venir de K003_reescalado, extraemos directamente la matriz sin volver a aplicar un Scaler
            features_scaled = df[features_cols].values
    
            # Ejecución de Clustering espacial basado en densidad
            ...
            # --- 3. METRICAS DE INTERFAZ ---
            with st.container(border=True):
                m1, m2, m3 = st.columns(3)
                m1.metric("Total Registros", len(df))
                m2.metric("Clusters (HGS)", len(df[df['HGS'] != 999]['HGS'].unique()))
                m3.metric("Zonas Complejas (Ruido)", int(df['Es_Zona_Compleja'].sum()), delta_color="inverse")

            with st.container(border=True):
                # --- 4. VISUALIZACIÓN 3D ---
                st.markdown("<h4 style='text-align: center;'>Visualización Espacial de Clusters</h4>", unsafe_allow_html=True)
          
                df['Etiqueta'] = df['HGS'].apply(lambda x: "Zona Compleja" if x == 999 else f"Cluster {x}")
                ...
                fig = px.scatter_3d(
                    ...
                    title=f"Distribución HGS (eps={val_eps}, min_samples={val_min_samples})",
                    labels={'Etiqueta': 'Clasificación HGS'},
                    opacity=0.7,
                    color_discrete_sequence=px.colors.qualitative.Safe
                )
            ...
            # --- 5. TABLA Y DESCARGA ---
            with st.expander("🔍 Ver vista previa de los datos etiquetados (Primeros 20 registros)"):
            ...
            with col_csv:
                ...
                st.download_button(
                    label="📥 Descargar K004_hgs.csv",
                    data=csv_data,
                    file_name='K004_hgs.csv',
                    mime='text/csv',
                    use_container_width=True
                )

            # --- 6. ALMACENAMIENTO DE SALIDA (Salida homologada a K004) ---
            st.session_state['K004_hgs'] = df.copy()
            st.caption("✅ Datos guardados con éxito en **K004_hgs**. Modifica los deslizadores laterales para actualizar automáticamente.")
                                 
        else:
            st.error(f"⚠️ Estructura inválida. Faltan columnas espaciales en el dataset reescalado. Se requiere: {features_cols}")
